...
const ModalCart = ({ serverTimeLeft }: { serverTimeLeft: CountdownTimeType }) => {
 ...
 <a
  href={`https://wa.me/51987847321?text=${encodeURIComponent(
    ...
    cartState.cartArray.map(item => `• ${item.name} (${item.selectedSize || item.sizes[0]}/${item.selectedColor || item.variation[0].color}) x${item.quantity} - $${item.price * item.quantity}`).join('\n') +
    `\n\nSubtotal: $${totalCart}.00`
  )}`}
  target="_blank"
  rel="noopener noreferrer"
  className="mt-4 w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-[#25D366] text-white text-sm font-semibold transition duration-300 hover:bg-[#1ebf57] shadow-md animate-pulse"
  onClick={closeModalCart}
>
